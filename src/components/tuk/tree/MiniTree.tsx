import { CATEGORIES, pastelOf } from "@/lib/tuk/constants";
...
export default function MiniTree({
  ...
  return (
    <svg viewBox="0 0 120 120" width={size} height={size} style={{ display: "block" }}>
      <defs>
        <filter id="softLeafMini" x="-40%" y="-40%" width="180%" height="180%">
      ...
      {Object.entries(CATEGORIES).map(([cat, meta]) => {
        ...
        return (
          <g key={cat}>
            ...
            <circle cx={bx} cy={by} r={9 + strength * 9} fill={soft} opacity="0.45" filter="url(#softLeafMini)" />
            {Object.entries(data.subs).map(([sub, cnt], i) => {
              const a = (i / Math.max(Object.keys(data.subs).length, 1)) * Math.PI * 2 + rad, rr = 5 + strength * 7;
              return <circle key={sub} cx={bx + Math.cos(a) * rr} cy={by + Math.sin(a) * rr} r={1.5 + Math.min(cnt, 6) * 0.7} fill={pastelOf(meta.color, 0.2)} stroke={fruitStroke} strokeWidth="0.5" />;
            })}
          </g>
        );
      })}
